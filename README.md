...
3. Set next environment variables for running `haproxy_agent.py` script:
    ...
    ```
4. Create folder `dev` at the root of repository, extract there content of the desired archive with the FRP archive which is located at `exapps_dev` folder.
...
3. Run `./development/debugging/redeploy_haproxy_host.sh` command to redeploy `app-harp` container **with HaProxy only**.

> **Note:** Existing `app-harp` container will be removed.
